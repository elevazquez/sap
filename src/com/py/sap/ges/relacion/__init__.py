...
@app.route('/relacion/nuevorelacion', methods=['GET', 'POST'])
def nuevorelacion():
    permission = UserPermission('administrador')
    if permission.can():
        form = RelacionFormulario(request.form)
        if request.method == 'POST' and form.validate():
            init_db(db_session)
            relacion = Relacion(form.codigo.data, form.descripcion.data, form.id_recurso.data)
            db_session.add(relacion)
            db_session.commit()
            flash('El relacion ha sido registrado con exito','info')
            return redirect('/relacion/administrarrelacion')
        return render_template('relacion/nuevorelacion.html', form=form)
    else:
...
def editarrelacion():
    init_db(db_session)
    p = db_session.query(Relacion).filter_by(codigo=request.args.get('codigo')).first()
    form = RelacionFormulario(request.form,p)
    relacion = db_session.query(Relacion).filter_by(codigo=form.codigo.data).first()
    if request.method == 'POST' and form.validate():
...
def eliminarrelacion():
    ...
    init_db(db_session)
    rol = db_session.query(Relacion).filter_by(codigo=cod).first()
...
def administrarrelacion():
    init_db(db_session)
    relacions = db_session.query(Relacion).order_by(Relacion.id)
    return render_template('relacion/administrarrelacion.html', relacions = relacions)
